...
rule(r'\bAIDS\b|\bHIV\b|\bSIDA\b', 'AIDS & HIV')
...
rule(r'\bborges\b', 'Jorge Luis Borges')
rule(r'\bcarlos fuentes\b|\bfuentes,\s*carlos\b', 'Carlos Fuentes')
rule(r'\boctavio paz\b|\bpaz,\s*octavio\b', 'Octavio Paz')
rule(r'\bjuan rulfo\b|\brulfo,\s*juan\b', 'Juan Rulfo')
rule(r'\bsor juana\b|\bjuana in[eé]s\b', 'Sor Juana Inés de la Cruz')
rule(r'\bluis zapata\b|\bzapata,\s*luis\b', 'Luis Zapata')
rule(r'\balfonso reyes\b|\breyes,\s*alfonso\b', 'Alfonso Reyes')
rule(r'\bcort[aá]zar\b', 'Julio Cortázar')
rule(r'\bgarc[ií]a m[aá]rquez\b', 'Gabriel García Márquez')
rule(r'\bneruda\b', 'Pablo Neruda')
rule(r'\blezama lima\b', 'José Lezama Lima')
rule(r'\breinaldo arenas\b|\barenas,\s*reinaldo\b', 'Reinaldo Arenas')
rule(r'\bsevero sarduy\b|\bsarduy\b', 'Severo Sarduy')
rule(r'\bcabrera infante\b', 'Guillermo Cabrera Infante')
rule(r'\bsade\b', 'Marquis de Sade')
rule(r'\bgenet\b', 'Jean Genet')
rule(r'\boscar wilde\b|\bwilde,\s*oscar\b', 'Oscar Wilde')
rule(r'\bproust\b', 'Marcel Proust')
rule(r'\bduras\b', 'Marguerite Duras')
rule(r'\brabelais\b', 'François Rabelais')
...
rule(r'\bnovo,?\s*salvador', 'Luis Zapata')  # group with Mexican queer writers
...
rule(r'\bsor juana\b', 'Sor Juana Inés de la Cruz')
...
rule(r'\bzapata quiroz\b', 'Luis Zapata')
